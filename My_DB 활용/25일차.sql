...
-- 다시풀어도 실수한 부분
--1)group by도 빼먹음 -> 없으면 그룹함수 count 사용 못함
--2)team_id = 'K02' or 'K09'
-- or 논리연산자는 연산순서중 가장 늦다
-- or 로 팀아이디를 연결하면 오류남
--3)count(team_id)을 하면 나중에 팀이름이 없는 인원수는 조회되지않는다
...
--2. EMP테이블의 자료를 사원번호를 기준으로 내림차순으로 정렬하여 
--사원번호와 사원명 컬럼을 표시하세요
--(단, 컬럼명은 영어로 표시한다)
...
--차이점
--오라클은 from 에서 , 콤마로 -  조인하고싶은 테이블 끼리 조인됨
--안시표준은 from에서 inner join 으로 - 조인하고싶은 테이블 끼리 조인됨

--오라클은 where절에 어느 컬럼이 같은지 적어줌
--안시표준은 on절에서 어느 컬럼이 같은지 적어줌

--오라클은 추가조건이 있을때 where절 다음에 and절로 작성
--안시표준은 초가조건이 있을때 on절 다음에 where절로 작성 - 그 다음에 조건을 추가하고 싶을때 and 나 or 을 사용한다

--!!!조인할때는 무조건 테이블. 적어주고 컬럼명을 적는다!!!
select e.ename, d.dname
